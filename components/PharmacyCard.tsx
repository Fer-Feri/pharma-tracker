...
export default function PharmacyCard({
	...
	function handleDelete() {
		if (confirm('این داروخانه حذف شود؟')) onDelete();
	}
	...
	return (
		<>
				...
				<div className="flex items-center justify-between">
					<div className="flex items-center gap-2.5">
						...
						<span className="text-sm font-medium flex items-center gap-1.5">
							<i
								className={`ti ${pharmacy.type === 'health' ? 'ti-heart-rate-monitor' : 'ti-pill'} text-muted-foreground text-sm`}
							/>
							{pharmacy.name}
						</span>
					</div>
						...
						<div className="flex flex-col gap-2">
							{activeInvoices.map((inv) => (
								<InvoiceRow
									key={inv.id}
									invoice={inv}
									onSettle={() => onSettleInvoice(inv.id)}
			...
			{showEditModal && (
				<PharmacyModal
					initial={{ name: pharmacy.name, phone: pharmacy.phone, type: pharmacy.type }}
